...
def align_images(fov_path):
    ...
    for i in range(0, len(images)):
        ...
        bfp_channel = images[i][2]
        yfp_channel = images[i][3]
        dsred_channel = images[i][4]
        ...
        rotated_bfp_images.append(rotate_image(bfp_channel, rotational_offsets[i]))
    ...
    for i in range(0, len(translational_offsets)):
        ...
        translated_bfp_images.append(translate_image(rotated_bfp_images[i], translational_offsets[i]))
    ...
    return (translated_vis_images, translated_gfp_images, translated_bfp_images, translated_yfp_images, translated_dsred_images)

def save_stacks(vis, gfp, bfp, yfp, dsred, save_path, fov_name):
    ...
    bfp_con = io.concatenate_images(img_as_uint(bfp))
    tf.imsave(save_path + '/%s_bfp_stack.tif' % fov_name, bfp_con)
    print("bfp stack saved")
