...
def align_images(fov_path):
    ...
    rotated_gfp_images = []
    rotated_mko_images = []
    rotated_e2crimson_images = []

    index = 0 # again, progress bar index
    for i in range(0, len(images)):

        vis_channel = images[i][0]
        gfp_channel = images[i][1]
        mko_channel = images[i][2]
        e2crimson_channel = images[i][3]

        index = index + 1
        print("Rotating image %d of %d" % (index, len(images)))
        rotated_vis_images.append(rotate_image(vis_channel, rotational_offsets[i]))
        rotated_gfp_images.append(rotate_image(gfp_channel, rotational_offsets[i]))
        rotated_mko_images.append(rotate_image(mko_channel, rotational_offsets[i]))
        rotated_e2crimson_images.append(rotate_image(e2crimson_channel, rotational_offsets[i]))
    ...
    translated_gfp_images = []
    translated_mko_images = []
    translated_e2crimson_images = []
    ...
    for i in range(0, len(translational_offsets)):
        ...
        translated_gfp_images.append(translate_image(rotated_gfp_images[i], translational_offsets[i]))
        translated_mko_images.append(translate_image(rotated_mko_images[i], translational_offsets[i]))
        translated_e2crimson_images.append(translate_image(rotated_e2crimson_images[i], translational_offsets[i]))

    return (translated_vis_images, translated_gfp_images, translated_mko_images, translated_e2crimson_images)
